...
class Level(IScene):
    def __init__(self, level_information: dict) -> None:
        ...
        self.limit: dict = level_information["limit"]
        ...
        self.player.set_minimum_position([self.limit["min_x"], self.limit["min_y"]])
        self.player.set_maximum_position([self.limit["max_x"], self.limit["max_y"]])
        self.list_components: list[ILevelComponent] = []

        # region Camera
        self.camera = Camera()
        self.camera.set_entity(self.player)
        self.camera.set_minimum_position([self.limit["min_x"], self.limit["min_y"]])
        self.camera.set_maximum_position([self.limit["max_x"], self.limit["max_y"]])
        # endregion
    ...
    def draw(self, window: Surface) -> None:
        window.fill("black")
        for components in self.list_components:
            components.draw(window, self.camera.get_position())
        if self.player:
